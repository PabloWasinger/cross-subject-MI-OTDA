        ...
        if len(reg_e_grid) == 1:
            ...
            for reg_e in reg_e_grid:
                ...
                if innerkfold is None:
                    ...
                    if innerkfold == 'LOO':
                        loo = LeaveOneOut()
                        splitter = loo.split(X_target)
                    else:
                        splitter = (train_test_split(X_target, y_target, train_size=innerkfold['train_size'], stratify=y_target, random_state=100*kk) for kk in range(innerkfold['nfold']))

                    for split_data in splitter:
                        if innerkfold == 'LOO':
                            train_idx, test_idx = split_data
                            X_train, X_test_inner = X_target[train_idx], X_target[test_idx]
                            y_train, y_test_inner = y_target[train_idx], y_target[test_idx]
                        else:
                            X_train, X_test_inner, y_train, y_test_inner = split_data
                        ...
                        if np.isnan(X_source_transported).any() or np.isinf(X_source_transported).any():

                            print(f"DEBUG: Cleaning NaNs in fold {k} with reg {reg_e}") 
                            X_source_transported = np.nan_to_num(X_source_transported)
                ...
                for j, reg_cl in enumerate(reg_cl_grid):
                    ...
                    if innerkfold is None:
                        ...
                        for split_data in splitter:
                            ...
                            if np.isnan(X_source_transported).any() or np.isinf(X_source_transported).any():
                                print(f"DEBUG: Cleaning NaNs in fold {k} with reg {reg_e} and {reg_cl}") 
                                X_source_transported = np.nan_to_num(X_source_transported)
...
def cv_sinkhorn_backward(reg_e_grid, X_source, y_source, X_target, y_target, clf,
    ...
    for k in range(outerkfold):
            ...
            for reg_e in reg_e_grid:
                ...
                if innerkfold is None:
                    ...
                    for split_data in splitter:
                        ...
                        if np.isnan(X_test_transported).any() or np.isinf(X_test_transported).any():
                            print(f"DEBUG: Cleaning NaNs in fold {k} with reg {reg_e}") 
                            X_test_transported = np.nan_to_num(X_test_transported)
        ...
        if np.isnan(X_target_transported).any() or np.isinf(X_target_transported).any():
            print(f"DEBUG: Cleaning NaNs in fold {k} with reg {regu}") 
            X_target_transported = np.nan_to_num(X_target_transported)
...
def cv_grouplasso_backward(reg_e_grid, reg_cl_grid, X_source, y_source,
    ...
    for k in range(outerkfold):
        ...
        if np.isnan(X_target_transported).any() or np.isinf(X_target_transported).any():
            print(f"DEBUG: Cleaning NaNs in fold {k} with reg {regu[0]} and {regu[1]}") 
            X_target_transported = np.nan_to_num(X_target_transported)
